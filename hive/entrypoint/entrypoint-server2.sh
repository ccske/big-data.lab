...
wait_until_service_up "hadoop-master" "8088" || exit 1
wait_until_service_up "hive-metastore" "9083" || exit 1
...
wait_until_service_up "localhost" "10000" || { echo "HS2 Thrift failed"; kill -TERM "${HS2_PID}" || true; exit 1; }
wait_until_service_up "localhost" "10002" || { echo "HS2 UI failed"; kill -TERM "${HS2_PID}" || true; exit 1; }
